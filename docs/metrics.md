...
| データ | Fitbit 側での実質更新頻度 |
| :--- | :--- |
| 心拍数 / 歩数 / AZM | ~15 分 (watch sync 依存) |
| 呼吸数 / SpO2 (intraday) | 睡眠中ポイント、起床後にまとめて確定 |
| HRV | 睡眠中、起床後に 1 回 |
| 睡眠ログ | 起床後に 1 回 |
| 皮膚温 | 睡眠中の集計、起床後に 1 回 |
| 体重 / 体脂肪 | スケール使用時のイベント駆動 |
| カーディオスコア (VO2 Max) | 運動後にまれに更新 |
| デバイス情報 (バッテリー等) | 同期ごと |
...
## 3. Cron スケジュール (レートリミット 150 req/h の予算配分)

MVP は **Pull-only 構成** で実装する (Subscription は将来拡張)。

| Job | 頻度 | 取得対象 | エンドポイント数 | コスト (req/h) |
| :--- | :--- | :--- | ---: | ---: |
| High-frequency | 15 分 | 心拍 (intraday), アクティビティ (intraday), AZM | 3 | 12 |
| Hourly | 1 時間 | 呼吸数, SpO2 (intraday), デバイス情報 | 3 | 3 |
| Post-wake (8:00 JST) | 1 日 | 睡眠, HRV, 皮膚温, カーディオスコア, 安静時心拍 | 5 | ~0.2 |
| Body (低頻度 polling) | 4 時間 | 体重, 体組成 | 1 | 0.25 |
| Token refresh | 必要時 (~7h ごと) | OAuth refresh | 1 | ~0.15 |
| **合計** | | | | **~16 req/h** |

**150 req/h のクォータに対して使用率 ~10%**。429 リトライや UI からの ad-hoc リクエスト、新しいメトリクス追加にも余裕がある。
...
## 8. 将来拡張: Subscription (Webhook) 統合

MVP には含めないが、将来的に以下の構成で応答性を改善できる:

- 起床時の `sleep` webhook 受信 → 睡眠 + HRV + 皮膚温 + 呼吸数 + SpO2 をまとめて pull
- 体重スケール使用時の `body` webhook → weight 即時反映
- High-frequency の心拍 / 歩数のみ 15 分 polling を維持
- 平常時コスト見積: ~12 req/h、睡眠 / 体重の反映が低レイテンシ化

導入には Webhook の重複・順序保証の検討が必要。
