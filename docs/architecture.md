...
| サービス | 無料枠 | 用途 | 採用 |
| :--- | :--- | :--- | :---: |
| **Workers** | 100k req/日 | ロジック全体 (Cron + HTTP) | ✅ MVP |
| **Cron Triggers** | 無制限 (Workers 内) | 定期データ取得 | ✅ MVP |
| **D1** | 5 GB / 5M reads/日 / 100k writes/日 | 時系列・トークン保存 | ✅ MVP |
| **Workers Secrets** | 無制限 | Client Secret 等 (3 個) | ✅ MVP |
...
```
   ...
   │                                                           │
   │  ┌────────────────┐  ┌──────────────┐  ┌───────────────┐  │
   │  │ Cron Triggers  │  │ HTTP Handler │  │ Static Assets │  │
   │  │ - 15min        │  │ /api/*       │  │ (GUI SPA)     │  │
   │  │ - 1h           │  │ /metrics     │  │   /           │  │
   │  │ - daily 8:00   │  │              │  │   /history    │  │
   │  │ - 4h           │  │              │  │               │  │
...
| サービス | 役割 |
| :--- | :--- |
| **Worker (1 個)** | Cron 実行 + HTTP API + GUI 配信を 1 デプロイで担当。`wrangler deploy` 一発でデプロイ完了 |
...
| **Workers Secrets** | `FITBIT_CLIENT_ID` / `FITBIT_CLIENT_SECRET` / `FITBIT_REFRESH_TOKEN_SEED` の 3 個 |
| **Cron Triggers** | `metrics.md` で定義した 5 ジョブを `wrangler.toml` の `[triggers]` で宣言 |
...
5. スクリプトがトークンエンドポイントへ送信し、`refresh_token` / `fitbit_user_id` / `scope` を取得
...
| `[triggers] crons` | — | `*/15 * * * *`, `0 * * * *`, `0 23 * * *` (= 8:00 JST), `0 */4 * * *` |
...
| Secrets (CLI 投入) | `FITBIT_CLIENT_SECRET` | Fitbit OAuth クライアント秘密鍵 |
...
| `pnpm bootstrap` | OAuth を完走し refresh_token を表示 |
